...
/* v12 — explicit inventory vs My Garage + first-run onboarding */
...
(function(){
  ...
  function updateHomeState(){
    ...
    if(emptyButton){
      emptyButton.innerHTML=empty?'<span><strong>Build My Garage →</strong><small>Choose from the fully built BMW 330Ci, Porsche Cayman GT4, Mustang GT and Focus ST.</small></span>':'<span><strong>＋ Browse & manage vehicles</strong><small>Add another car or remove one from My Garage.</small></span>';
    }
  ...
  function renderManager(){
    ...
    manageGrid.innerHTML=data.map(v=>{
      ...
      return `<button class="gj-manage-card${on?' is-selected':''}" type="button" data-manage-key="${esc(v.key)}" style="--vehicle-accent:${esc(v.accent)}">
        <span class="gj-manage-toggle">${on?'✓ In My Garage':'+ Add to Garage'}</span>
        <div class="gj-manage-photo" style="${photoStyle(v)}"></div>
        <div class="gj-manage-copy"><div class="gj-manage-meta"><span>${esc(v.catalog_label)}</span><span>${esc(v.year)} / ${esc(mark(v))}</span></div><strong>${esc(v.make)} ${esc(v.model)}</strong><p>${esc(v.powertrain)}</p></div>
      </button>`;
    }).join('');
  ...
  const observer=new MutationObserver(decorateGarage);if(garageGrid)observer.observe(garageGrid,{childList:true,subtree:true});
  decorateGarage();
